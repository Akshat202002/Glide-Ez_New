...
def edit_account_details_view(request):
    if request.method == "POST":
        ...
        sweetify.success(request, 'Account Updated Successfully', button='Ok', timer=3000)
...
def payment_view(request):
    if request.method == "POST":
        ...
        for st in seat_list:
            name=request.POST.get(st+'name')
            email=request.POST.get(st+'email')
            dob=request.POST.get(st+'DOB')
            phone=request.POST.get(st+'phone')
            passengers[st]=[name,email,dob,phone]
        print(passengers)

        return render(request, 'glideEz/payment.html',{'price': total_price,'seat_list':seat_list,'trip_id':trip_id,'passengers':passengers})
...
def payment_redirect_view(request):
    if request.method=="POST":
        ...
        print(total_price)
        mycursor = mydb.cursor()
        str="""select flight_id,src_id,dest_id from trip where trip_id={}""".format(trip_id)
        mycursor.execute(str)
        ...
        for seat in seat_list:
            ...
            mycursor.execute(str)
            pass_id=mycursor.fetchall()
            pass_id=pass_id[0][0]
            pas = passengers[seat]
            str="""insert into Passenger values(null,{},'{}', '{}', {}, '{}', {}, '{}');""".format(x,pas[0], pas[2], pas[3], pas[1], trip_id, seat)
            mycursor.execute(str)
            str="""insert into ticket values(null,{},{},{},{},{},1,{},'{}');""".format(flight_id,trip_id,x,src_id,dest_id,pass_id,seat)
            mycursor.execute(str)
            mydb.commit()
...
def cancelbooking(request):
    ...
    print(placeholders)
    str="""update seat set busy=false where trip_id={} and seat_no in {};""".format(trip_id,placeholders)    
    mycursor.execute(str)
    str="""update booking set booking_status = 0 where booking_id={};""".format(booking_id)
    mycursor.execute(str)
    mydb.commit()
    return redirect('bookings')
